app.controller('userListCtrl', function ($scope, $rootScope, $http, $window) {
	$scope.listUser = [];

	$rootScope.get($rootScope.listUserApi).success(function (data, status, headers, config) {
	...
	$scope.currentPage = 0;
	$scope.pageSize = 10;
	$scope.pageCount = Math.ceil($scope.listUser.length / $scope.pageSize);
...
window.Parsley.addValidator('fullName', {
	...
	messages: {
		vi: 'Họ và tên lớn hơn hoặc bằng 10 ký tự.'
	}
});
